...
headers = {
    'Cookie': nonebot.get_driver().config.oba_cookie,
}
...
由于 Lagrange 暂时无法发送临时消息，请不要使用临时消息请求回复。
特别鸣谢: 盐木、甜木、米露、听风、天秀 和 bangbang93 的不杀之恩
''')
    
# OpenBMCLAPI 总览
status = on_command("面板")
@status.handle()
...
async def handle_function(bot: Bot, event: Event, args: Message = CommandArg()):
    reply_msg = Message(f'[CQ:reply,id={event.message_id}]')
    args = str(args).replace('\n', '')
    if str(args) == '' or str(args).isspace():
        await node.finish(reply_msg + '缺参数啦！记得补上喵喵～')
    elif len(str(args)) > 16:
        await node.finish(reply_msg + f'''OpenBMCLAPI 面板数据 {plugin_version}
要求: 节点名称 最多 16 个字符
...
当日流量: {hum_convert(match.get('metric', {}).get('bytes', 0))} | 当日请求数: {format_number(match.get('metric', {}).get('hits', 0))} 次'''
                send_text += f'\n请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
            await node.finish(reply_msg + send_text)
...
请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'''
            await node.finish(send_text)
        else:
            await node.finish(reply_msg + f'''未找到有关 {args} 的相关节点信息，请调整参数后重新尝试
请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}''')
...
async def handle_function(bot: Bot, event: Event, args: Message = CommandArg()):
    ...
    args = str(args).replace('\n', '')
    if str(args) == '' or str(args).isspace():
        await node_id.finish(reply_msg + '缺参数啦！记得补上喵喵～')
    elif len(str(args)) > 24:
        await node_id.finish(reply_msg + f'''OpenBMCLAPI 面板数据 {plugin_version}
要求: 节点ID 最多 24 个字符
搜索条件不符合要求，请调整参数后重新尝试''')
    else:
        send_text = f'OpenBMCLAPI 面板数据 {plugin_version}'
...
ID: {match.get('_id')}'''
                send_text += f'\n请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
            await node_id.finish(reply_msg + send_text)
...
请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'''
            ...
            await node_id.finish(reply_msg + f'''未找到有关 {args} 的相关节点信息，请调整参数后重新尝试
请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}''')
...
当日请求数: {format_number(match.get('metric', {}).get('hits', 0))} 次'''
            ...
            send_text += f'\n索引超出范围，请输入一个有效的数字。'
            send_text += f'\n请求时间: {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
            await node_rank.finish(reply_msg + send_text)
    except ValueError:
        if str(args) == '' or str(args).isspace():
            await node_rank.finish(reply_msg + '缺参数啦！记得补上喵喵～')
        else:
            await node_id.finish(reply_msg + f'''OpenBMCLAPI 面板数据 {plugin_version}
要求: 节点名次 必须为一个整数
搜索条件不符合要求，请调整参数后重新尝试''')
...
async def handle_function(bot: Bot, event: Event, args: Message = CommandArg()):
    ...
    args = str(args).replace('\n', '')
    if str(args) == '' or str(args).isspace():
        await bangbang93HUB.finish(reply_msg + MessageSegment.image("https://apis.bmclapi.online/api/93/random"))
    else:
        matchList = []
        ...
        if len(matchList) < 1:
            await bangbang93HUB.finish(reply_msg + '找不到哦，请重新尝试~')
        elif len(matchList) == 1:
            await bangbang93HUB.finish(reply_msg + MessageSegment.image(f"https://apis.bmclapi.online/api/93/file?name={matchList[0]}"))
        else:
            await bangbang93HUB.finish(reply_msg + f'搜索结果包含 {len(matchList)} 条，请改用更加精确的参数搜索')
